...
import { Download, TrendingUp, Users, ShoppingCart, BookOpen } from 'lucide-vue-next';
...
function changeYear(y: number) {
    router.get('/admin/reports', { year: y }, { preserveState: true, replace: true });
}
        ...
        <div class="max-w-7xl space-y-8 p-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Data aktivitas platform DifaFriends</p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
                <div
                    v-for="card in [
                        { label: 'Total Revenue', value: formatRp(summary.total_revenue), icon: TrendingUp, color: 'purple' },
                        { label: 'Order Berhasil', value: summary.total_orders, icon: ShoppingCart, color: 'blue' },
                        { label: 'Pendaftaran Kelas', value: summary.total_enrollments, icon: BookOpen, color: 'green' },
                        { label: 'User Baru', value: summary.new_users, icon: Users, color: 'amber' },
                    ]"
                    :key="card.label"
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                <h2 class="mb-6 font-semibold text-gray-900 dark:text-white">Revenue per Bulan (Rp)</h2>
                <div class="flex h-48 items-end gap-1.5">
                    ...
                    >
                        ...
                        <div
                            ...
                            :style="{ height: barHeight(item.value, revenueMax) }"
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <!-- Enrollments -->
                <div class="rounded-2xl border border-gray-100 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <h2 class="mb-6 font-semibold text-gray-900 dark:text-white">Pendaftaran Kelas per Bulan</h2>
                    <div class="flex h-36 items-end gap-1.5">
                        ...
                        >
                            ...
                            <div
                                ...
                                :style="{ height: barHeight(item.value, enrollMax) }"
                                style="min-height: 4px"
                            />
                            <span class="text-xs text-gray-400">{{ item.month }}</span>
                        </div>
                ...
                <!-- User Growth -->
                <div class="rounded-2xl border border-gray-100 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <h2 class="mb-6 font-semibold text-gray-900 dark:text-white">Pengguna Baru per Bulan</h2>
                    <div class="flex h-36 items-end gap-1.5">
                        <div
                        ...
                        >
                            ...
                            <div
                                ...
                                :style="{ height: barHeight(item.value, userMax) }"
                                style="min-height: 4px"
                            />
                            <span class="text-xs text-gray-400">{{ item.month }}</span>
                        </div>
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                <h2 class="mb-6 font-semibold text-gray-900 dark:text-white">Top 10 Kelas — Revenue Tertinggi</h2>

                <div v-if="topCourses.length === 0" class="py-10 text-center text-sm text-gray-400">
                    Belum ada data.
                </div>
                ...
                <div v-else class="space-y-3">
                    ...
                    >
                        <span class="w-5 shrink-0 text-right text-sm font-bold text-gray-400">{{ i + 1 }}</span>
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="ml-4 shrink-0 font-semibold text-purple-600">
                            ...
                            <div class="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                                <div
                                    ...
                                    :style="{ width: `${(course.total_revenue / topRevenueMax) * 100}%` }"
                                />
                            </div>
                            <p class="mt-0.5 text-xs text-gray-400">{{ course.total_orders }} transaksi</p>
                        </div>
                    </div>
                </div>
            </div>
