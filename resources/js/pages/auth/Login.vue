...
const passwordUp = computed(() => passwordFocus.value || password.value.length > 0);
...
async function submit() {
    ...
    try {
        ...
        serverErr.value = (errs.email && errs.email[0])
...
<template>
    <AuthBase>
        ...
        <div class="mb-8">
            ...
            <h1 class="text-[26px] font-bold tracking-tight text-gray-900 dark:text-white">
                Selamat datang
            </h1>
        ...
        <!-- Status -->
        <div v-if="status" class="mb-5 flex items-center gap-2 rounded-xl bg-green-50 px-4 py-3 text-sm text-green-700 ring-1 ring-green-200 dark:bg-green-900/20 dark:text-green-400 dark:ring-green-800/40">
        ...
        <!-- Server error -->
        <div v-if="serverErr" class="mb-5 flex items-start gap-2.5 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-600 ring-1 ring-red-200 dark:bg-red-900/20 dark:text-red-400 dark:ring-red-800/40">
        ...
        <form @submit.prevent="submit" novalidate class="flex flex-col gap-3">
            ...
            <div class="group relative">
                <input
                    ...
                    @blur="emailFocus = false; emailTouched = true"
                    :class="[
                        'w-full rounded-xl border px-4 pb-2.5 pt-[22px] text-sm text-gray-900 outline-none transition-all duration-150',
                        'dark:text-white',
                        emailErr
                ...
                <label
                    ...
                    :class="[
                        ...
                        emailUp
                            ? 'top-[7px] text-[10px] ' + (emailErr ? 'text-red-400' : emailFocus ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500')
                ...
                <div class="pointer-events-none absolute inset-y-0 right-3.5 flex items-center">
                    <transition name="fade">
                        <svg v-if="emailTouched && email && !emailErr" class="h-4 w-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                        </svg>
                    </transition>
                </div>
                <!-- Error -->
                <p v-if="emailErr" class="mt-1.5 flex items-center gap-1 px-1 text-[11px] text-red-500">
            ...
            <div class="relative">
                <input
                    ...
                    @blur="passwordFocus = false; passTouched = true"
                    :class="[
                        'w-full rounded-xl border px-4 pb-2.5 pr-11 pt-[22px] text-sm text-gray-900 outline-none transition-all duration-150',
                        'dark:text-white',
                        passErr
                            ? 'border-red-300 bg-red-50/30 focus:border-red-400 focus:ring-2 focus:ring-red-300/20 dark:border-red-700/60 dark:bg-red-900/10'
                            : passwordFocus
                ...
                <label
                    ...
                    :class="[
                        'pointer-events-none absolute left-4 font-medium transition-all duration-150',
                        passwordUp
                            ? 'top-[7px] text-[10px] ' + (passErr ? 'text-red-400' : passwordFocus ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500')
                            : 'top-1/2 -translate-y-1/2 text-sm text-gray-400 dark:text-gray-500',
                    ]"
                >
                    Kata Sandi
                ...
                >
                    <svg v-if="!showPass" class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Error -->
                <p v-if="passErr" class="mt-1.5 flex items-center gap-1 px-1 text-[11px] text-red-500">
                    <svg class="h-3 w-3 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                    </svg>
                    {{ passErr }}
            ...
            <div class="flex items-center justify-between pt-0.5">
                <label class="flex cursor-pointer items-center gap-2">
                    ...
                    >
                        <svg v-if="remember" :class="['h-2.5 w-2.5', 'text-white dark:text-gray-900']" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
                ...
                <svg v-if="!loading" class="h-4 w-4 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative my-1">
                ...
                <div class="relative flex justify-center">
                    <span class="bg-white px-3 text-[11px] text-gray-400 dark:bg-[#0a0a0a] dark:text-gray-600">
            ...
            <!-- ── Register ───────────────────────────────────────── -->
            <p v-if="canRegister" class="pt-1 text-center text-sm text-gray-400 dark:text-gray-500">
                Belum punya akun?
                <Link :href="register()" :tabindex="6" class="font-semibold text-gray-900 underline-offset-2 hover:underline dark:text-white">
                    Daftar gratis
                </Link>
            </p>
        </form>
