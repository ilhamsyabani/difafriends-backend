    ...
    <AppLayout>
        ...
        <div class="max-w-7xl space-y-6 p-6">
            <div>
                ...
                <p class="mt-1 text-sm text-muted-foreground">Pantau performa kelas kamu</p>
            </div>

            <!-- Stats -->
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <StatsCard label="Total Kelas" :value="stats.totalCourses" color="brand" />
            ...
            <div class="overflow-hidden rounded-2xl border border-border bg-card">
                <SectionHeader title="Kelas Saya" href="/instructor/courses" link-text="Kelola semua" />

                <EmptyState v-if="stats.courses.length === 0" message="Belum ada kelas.">
                    <template #action>
                        <Link href="/instructor/courses/create" class="text-sm text-brand hover:underline">
                            Buat kelas pertama →
                        </Link>
                    </template>
                </EmptyState>
